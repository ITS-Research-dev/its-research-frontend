...
export function formatIntoProfileSummary(
  ...
  datas.map((data) => {
    ...
    if (!competencyTrend[week])
      competencyTrend[week] = { total: { ...TEMPLATE } };
    if (!competencyTrend[month])
      competencyTrend[month] = { total: { ...TEMPLATE } };
    if (!competencyTrend[week][topicTitle])
      competencyTrend[week][topicTitle] = { ...TEMPLATE };
    if (!competencyTrend[month][topicTitle])
      competencyTrend[month][topicTitle] = { ...TEMPLATE };
